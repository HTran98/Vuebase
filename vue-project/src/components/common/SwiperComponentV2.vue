...
<template>
  <div class="swiper_custom">
    ...
    <div class="custom">
      <swiper :modules="modules" class="mySwiper" @swiper="onSwiper" ref="swiperRef" :navigation="false" @slideChange="handleSlideChange">
        <swiper-slide v-for="i in totalSlide">
          <div class="search_box card col_3">
            <ul>
              <li v-for="i in listItemCard">
                <ItemCard class="class_test" border-shape="normal" :is-show-icon="true">
                  <template #status>
                  ...
                  <template #body>
                    <div class="card_custom_content">
                      <div class="datetime">
                        ...
                        <div class="time" v-if="i.startDate && i.endDate">{{ i.startDate }}~{{ i.endDate }}</div>
                      </div>
                    </div>
                  </template>
                </ItemCard>
      ...
      <PaginationComponent class="swiper_custom__pagination" :isBounded="true" :total-record="100" :pageSize="showPagesize" v-model="activePage" />
    </div>
